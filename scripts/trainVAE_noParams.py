...
def main():
    ...
    params['num_workers'] = 128
    params['output_dir'] = '/n/holyscratch01/iaifi_lab/agagliano/galaxyAutoencoder/models/fullTrain_crop_noParams/' #decals_checkpoints/'
    params['ckpt_path'] = ''
    ...
    with open('validatedResults_crop_fullTrain_noParams.pkl', 'wb') as handle:
...
def ELBO(x_hat, x, mu, logvar, y, beta0, beta1, beta2, device):
    y = y.squeeze()#.to(device) #remove mysterious middle dimension
    x = x#.to(device)
